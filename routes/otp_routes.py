...
from helpers.status_codes import (
    STATUS_CODE_MISSING_FIELDS_400,
    STATUS_CODE_500,
    STATUS_CODE_404,
)
...
async def validate_otp(req: OTPVerification):
    ...
    otp_log = await repository.get_one(
        otp_collection, {"phone": req.phoneNumber, "otp": req.otp}
    )

    if not otp_log:
        return STATUS_CODE_404

    is_valid = await verify_otp(otp_log)
    ...
    try:
        updated_result = await otp_collection.update_one(
            {"phone": req.phoneNumber, "otp": req.otp}, {"$set": {"is_valid": False}}
        )
